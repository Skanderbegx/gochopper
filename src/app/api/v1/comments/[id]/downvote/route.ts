...
export async function POST(
  ...
  if (existing) {
    if (existing.value === -1) {
      ...
      return jsonSuccess({ message: "Downvote removed", vote: 0 });
    }
    await prisma.vote.update({ where: { id: existing.id }, data: { value: -1 } });
    await prisma.comment.update({ where: { id: commentId }, data: { score: { decrement: 2 } } });
    return jsonSuccess({ message: "Changed to downvote", vote: -1 });
  }
  ...
  return jsonSuccess({ message: "Downvoted", vote: -1 });
}
